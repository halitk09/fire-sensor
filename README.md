...
├── fire_sensor.launch
├── fire_sensor_with_mockadcsensor_pythongui.rar   # mock ADC + Python gui (ayrı proje)
├── README.md
...
- **Log:** `Docs/pythonguilog.txt`  
- **Mock + bench:** Bu repodaki üretim `Core/` ağacında mock yoktur. Mock ADC, LPUART (ST-Link VCP) ve Python masaüstü aracı **`fire_sensor_with_mockadcsensor_pythongui.rar`** arşivindeki paralel Cube projesinde bulunur.
